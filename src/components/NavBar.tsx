    ...
    async function handleInputChange(value: string) {
        ...
        if (value.length >= 3) {
            try {
                ...
                const suggestions = response.data.list.map((city: { name: string }) => city.name);
    ...
    return (
        <>
            <nav className="shadow-sm sticky top-0 left-0 z-50 bg-white">
                <div className="h-[4em] w-full flex justify-between items-center max-w-7xl px-3 mx-auto">
                    ...
                    <section className="flex gap-2 items-center">
                        <MdMyLocation
                            ...
                            className="text-2xl text-gray-400 hover:opacity-50 cursor-pointer" />
                        <MdOutlineLocationOn className="text-3xl" />
                        <p className="text-slate-900/80 text-sm">{location}</p>
                        <div className="relative hidden md:flex">
                            <SearchBox
                                value={city}
                                onChange={(e) => handleInputChange(e.target.value)} onSubmit={handleSubmitSearch}
                            />
                            <SuggestionBox
                ...
                <section className="flex max-w-7xl px-3 md:hidden justify-center align-center">
    ...
    return (
        <>
            {(showSuggestions && suggestions.length > 0 || error) && (
                <ul className="mb-4 bg-white absolute border top-11 left-0 border-gray-300 rounded-md min-w-50 flex flex-col gap-1 py-2 px-2">
